...
export async function ensureDemoUser(): Promise<number> {
  ...
  const [existing] = await db
    ...
    .limit(1);
  if (existing) {
    // Samonaprawa: jeśli konto demo powstało w starszej wersji aplikacji
    // (inny format hasha lub inne hasło), przywróć poprawne dane logowania.
    if (!(await verifyPassword("demo1234", existing.password))) {
      await db
        .update(users)
        .set({ password: await hashPassword("demo1234") })
        .where(eq(users.id, existing.id));
    }
    return existing.id;
  }

  // Insert with conflict protection: concurrent visits to /login (prefetch +
  // navigation) must not crash on a duplicate e-mail race.
  ...
  const [created] = await db
    ...
    .onConflictDoNothing()
    .returning();
  const userId = created?.id ?? (await db.select({ id: users.id }).from(users).where(eq(users.email, email)).limit(1))[0]?.id;
  if (!userId) return 0;

  try {
    await seedStarterData(userId);
  } catch (error) {
    // Seeding is non-critical: catalog is re-ensured on the next app page.
    console.error("seedStarterData failed for demo user:", error);
  }
  return userId;
}
