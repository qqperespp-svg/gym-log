...
export default async function DashboardPage({
  ...
  const goal = 4;
  const progress = Math.min(100, Math.round((weekly.length / goal) * 100));
          ...
          <article className="panel p-6">
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-2xl font-black text-white">
                  {weekly.length} <span className="text-sm font-medium text-slate-600">/ {goal} treningi</span>
                </p>
              </div>
            ...
            <p className="mt-3 text-xs text-slate-500">
              {progress >= 100
                ...
                : `Jeszcze ${Math.max(0, goal - weekly.length)} — dasz radę.`}
            </p>
          </article>
          <article className="panel p-6">
