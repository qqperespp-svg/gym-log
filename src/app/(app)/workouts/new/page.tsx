...
export default async function NewWorkoutPage({
  ...
  const [library, programRows, lastPerformance, query] = await Promise.all([
    ...
    db
      .select({
        ...
        definitionId: programExercises.exerciseDefinitionId,
        exerciseName: programExercises.name,
    ...
    programRows
      .reduce((map, row) => {
        const current = map.get(row.id) ?? {
          ...
          exercises: [] as ProgramItem["exercises"],
        };
        if (row.exerciseId && row.definitionId)
          current.exercises.push({
            definitionId: row.definitionId,
            name: row.exerciseName ?? "",
            targetSets: row.targetSets ?? 3,
            targetReps: row.targetReps ?? 10,
  ...
  const requestedProgram = Number(query.program);
  const initialProgramId = programs.some((item) => item.id === requestedProgram) ? requestedProgram : 0;
  return (
    <div className="space-y-7">
      <header>
        ...
        <p className="eyebrow">Nowa sesja</p>
        <h1 className="page-title">Zaplanuj trening</h1>
        <p className="mt-2 text-sm text-slate-500">Wybierz program lub zbuduj sesję od zera.</p>
      </header>
      <WorkoutForm
