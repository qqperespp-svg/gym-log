    ...
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-amber-950 px-6">
      <form action={formAction} className="w-full max-w-md space-y-6 bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-3xl p-10 shadow-2xl shadow-amber-900/20">
        <div className="space-y-2 text-center">
          ...
          <p className="text-slate-400 text-sm">Join the pack and start tracking every rep.</p>
        </div>
        ...
        <div className="space-y-4">
          <input name="name" placeholder="Full name" required className="w-full rounded-xl bg-slate-800/60 border border-slate-600 text-white px-5 py-3 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 transition" />
          ...
          <input name="password" type="password" placeholder="Password (min. 6 chars)" required className="w-full rounded-xl bg-slate-800/60 border border-slate-600 text-white px-5 py-3 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 transition" />
          <button type="submit" disabled={pending} className="w-full rounded-xl bg-amber-600 hover:bg-amber-500 disabled:opacity-60 text-white font-bold py-3 transition shadow-lg shadow-amber-900/30 flex items-center justify-center gap-2">
            {pending ? <Loader2 className="w-4 h-4 animate-spin" /> : null} Create account
          </button>
        </div>

        <p className="text-sm text-center text-slate-500">
          Already have an account?{" "}
          <Link href="/login" className="text-amber-400 font-semibold hover:underline">Log in</Link>
        </p>
      </form>
    </main>
  );
