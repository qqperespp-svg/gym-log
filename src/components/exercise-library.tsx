...
export function ExerciseLibrary({ exercises }: { exercises: Item[] }) {
  ...
  const [optimistic, removeOptimistic] = useOptimistic(exercises, (items, id: number) =>
  ...
  const groups = useMemo(
    () => [
      "Wszystkie",
      ...Array.from(new Set(optimistic.map((item) => item.muscleGroup))).sort((a, b) =>
  ...
  return (
    <div className="grid items-start gap-6 xl:grid-cols-[360px_1fr]">
      <form action={formAction} className="panel p-6 xl:sticky xl:top-6">
        <div className="mb-6 flex items-center gap-3">
          <span className="icon-box">
            <Plus size={20} />
          </span>
          <div>
            <h2 className="font-extrabold text-white">Własne ćwiczenie</h2>
            <p className="text-sm text-slate-500">Dodaj ruch spoza katalogu</p>
          </div>
        </div>
        <div className="space-y-4">
          <label className="field-label">
            Nazwa
            <input className="input" name="name" placeholder="np. Wiosłowanie Pendlay" required />
          </label>
          <label className="field-label">
            Partia mięśniowa
            <select className="input" name="muscleGroup" defaultValue="" required>
          ...
          <label className="field-label">
            Sprzęt
            <select className="input" name="equipment" defaultValue="Sztanga">
              {equipment.map((item) => (
                <option key={item}>{item}</option>
              ))}
            </select>
          ...
          <SubmitButton pendingLabel="Dodawanie…" className="button-primary w-full justify-center">
            <Plus size={17} /> Dodaj do biblioteki
          </SubmitButton>
        </div>
      </form>
      ...
      <section>
        <div className="mb-4">
          <h2 className="font-extrabold text-white">Katalog ćwiczeń</h2>
          <p className="text-sm text-slate-500">
            {optimistic.length} ruchów · katalog bazowy i Twoje własne
          </p>
        </div>
        <div className="mb-5 flex flex-col gap-3 sm:flex-row">
          <span className="input-shell flex-1">
            <Search size={17} />
            <input
              value={search}
          ...
          <select className="input sm:w-52" value={group} onChange={(event) => setGroup(event.target.value)}>
            {groups.map((item) => (
              <option key={item}>{item}</option>
            ))}
          </select>
        ...
        {!visible.length ? (
          <div className="empty-state">
            ...
            <h3>Brak wyników</h3>
            <p>Zmień wyszukiwaną frazę lub filtr partii mięśniowej.</p>
          </div>
        ) : (
              ...
              <article key={item.id} className="panel flex items-center gap-4 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {item.isCustom === 1 && (
                      <span className="rounded bg-sky-400/10 px-1.5 py-0.5 text-[8px] font-black uppercase text-sky-300">
                        Własne
                      </span>
                    )}
                  </div>
                  <p className="mt-1 text-xs text-slate-500">
                    {item.muscleGroup} <span className="mx-1 text-slate-700">•</span> {item.equipment}
                  </p>
                </div>
                {item.isCustom === 1 && (
                  <>
                    <Link href={`/exercises/${item.id}/edit`} className="icon-button">
                      <Edit3 size={16} />
                    </Link>
